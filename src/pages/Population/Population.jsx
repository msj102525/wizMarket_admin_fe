...
const Population = () => {
  ...
  const mergeDataPairs = (data) => {
    ...
    for (let i = 0; i < data.length; i += 2) {
      ...
      const total_population = (first.total_population || 0);

      // 남자와 여자의 비율 계산
      const male_population = (first.total_population_by_gender || 0)

      const female_population = (second.total_population_by_gender || 0)

      const db_age_under_10s = (first.age_under_10s || 0) + (second.age_under_10s || 0)
  ...
  const handleSearch = async (filters) => {
    ...
    try {
      ...
      // 성별 필터가 없으면 데이터를 합침
      const mergedResults =  mergeDataPairs(response.data.filtered_data);
